...
## QC
```{r}
# 1. remove probe with missing value > 50% samples 
meth270 <- meth270[rowMeans(is.na(meth270)) < 0.5, ]
meth450 <- meth450[rowMeans(is.na(meth450)) < 0.5, ]
meth_LGG <- meth_LGG[rowMeans(is.na(meth_LGG)) < 0.5, ]

# 2. remove probe from chrX, chrY probe
...
venn.plot <- venn.diagram(
  ...
  filename = paste0(outpath,"Probes_venn_plot.png"),
  imagetype = "png",
...
gene_meth270 <- get_gene_meth(meth270_filtered, probe_gene_df)
gene_meth450 <- get_gene_meth(meth450_filtered, probe_gene_df)
gene_methLGG <- get_gene_meth(meth_LGG_filtered, probe_gene_df)


# remove genes with missing value > 50% samples 
gene_meth270_filtered <- gene_meth270[rowMeans(is.na(gene_meth270)) < 0.5, ]
gene_meth450_filtered <- gene_meth450[rowMeans(is.na(gene_meth450)) < 0.5, ]
gene_methLGG_filtered <- gene_methLGG[rowMeans(is.na(gene_methLGG)) < 0.5, ]
...
venn.plot <- venn.diagram(
  x = list( "270K" = set1,"450K" = set2,"LGG" = set3),
  filename = paste0(outpath,"Methy_genes_venn_plot.png"),
  imagetype = "png",
  fill = c("cornflowerblue", "darkorange","lightyellow"),
  alpha = 0.5,
  cex = 1.5,
...
ggsave(paste0(outpath,"pca_methylation_batch_before.png"), plot = p_before, width = 6, height = 5, dpi = 300)
```
...
ggsave(paste0(outpath,"pca_methylation_batch_corrected.png"), plot = p_after, width = 6, height = 5, dpi = 300)
```
